...
Litecoin Core 0.15.0 contains the following changes to the RPC interface and `dancoin-cli` for multi-wallet:

* When running Litecoin Core with a single wallet, there are **no** changes to the RPC interface or `dancoin-cli`. All RPC calls and `dancoin-cli` commands continue to work as before.
